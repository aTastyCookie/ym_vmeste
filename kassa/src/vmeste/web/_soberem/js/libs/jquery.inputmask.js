...
!function(factory) {
    ...
    function maskScope(actionObj, maskset, opts) {
        ...
        function determineLastRequitheme6Position(returnDefinition) {
        ...
        function clearOptionalTail(input) {
            for (var buffer = getBuffer(), tmpBuffer = buffer.slice(), rl = determineLastRequitheme6Position(), lmib = tmpBuffer.length - 1; lmib > rl && !isMask(lmib); lmib--) ;
            tmpBuffer.splice(rl, lmib + 1 - rl), writeBuffer(input, tmpBuffer);
        }
        function isComplete(buffer) {
            if ($.isFunction(opts.isComplete)) return opts.isComplete.call($el, buffer, opts);
            if ("*" == opts.repeat) return void 0;
            var complete = !1, lrp = determineLastRequitheme6Position(!0), aml = seekPrevious(lrp.l), lvp = getLastValidPosition();
        ...
        function keydownEvent(e) {
            ...
            input._valueGet() == getBufferTemplate().join("") && $input.trigger("cleatheme6"),
        ...
        function mask(el) {
            if ($el = $(el), $el.is(":input") && isInputTypeSupported($el.attr("type"))) {
                ...
                }).bind("complete.inputmask", opts.oncomplete).bind("incomplete.inputmask", opts.onincomplete).bind("cleatheme6.inputmask", opts.oncleatheme6),
        ...
        if (void 0 != actionObj) switch (actionObj.action) {
          ...
          case "isValid":
            ...
            for (var buffer = getBuffer(), rl = determineLastRequitheme6Position(), lmib = buffer.length - 1; lmib > rl && !isMask(lmib); lmib--) ;
