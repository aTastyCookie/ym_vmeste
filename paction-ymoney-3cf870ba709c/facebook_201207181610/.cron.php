       ...
       if (!$ignore) {
           ...
           foreach ($alloperations as $operation) {
               ...
               if ($lastOperationIdHistory != $OperationId) {
                   ...
                   //выбираем необходимый пост
//                    $result = $usersModel->select()->where('id = ?', $action['user_id']);
//                    $user = $usersModel->fetchRow($result);
//                    $userToken = $user->token;
//                    $result = $facebook->api('/me/feed', 'GET', array('access_token' => $userToken));
//                    if(!isset($result['error'])) {
//                        $pid = 0;
//                        if(!empty($result['data'])) {
//                            foreach($result['data'] as $k=>$v) {
//                                if(isset($v['message'])
//                                        && $v['message'] == 'Статистика приложения "Собирайте деньги" по акции "' . $action['name'] . '"'){
//                                    $pid = $v['id'];
//                                    break;
//                                }
//                            }
//                        }
//                        // Если поста нет, то создаем
//                        if($pid == 0) {
//                            $post_details = array(
//                                'message' => 'Статистика приложения "Собирайте деньги" по акции "' . $action['name'] . '"',
//                                'name' => $config->ymoney->APP_NAME,
//                                'link' => $config->ymoney->APP_PAGE,
//                                'caption' => 'apps.facebook.com',
//                                'picture' => $config->ymoney->APP_PAGE . "/i/app_75.png",
//                                'access_token' => $userToken
//                            );
//                            $create_post = $facebook->api('/me/feed', 'post', $post_details);
//                            $pid = $create_post['id'];
//                        }
//
//                        //оставляем сообщение
...
//                                'message' => 'Были перечислены деньги в размере ' . $op['amount'] . ' руб.',
//                                'access_token' => $userToken
//                            );
//                            $create_comment = $facebook->api('/' . $pid . '/comments', 'post', $comment_details);
//                            $cid = $create_comment['id']; 
//                        }
//                    }
...
//                                        'link' => $config->ymoney->APP_PAGE,
//                                        'caption' => 'apps.facebook.com',
//                                        'picture' => $config->ymoney->host . "/i/app_75.png",
//                                        'access_token' => $userToken
//                                    );
//                                    $create_post = $facebook->api('/me/feed', 'post', $post_details);
//                                    $pid = $create_post['id'];
...
//                            }
   ...
   $current_sum = 0;
   $count = 0;
   $daycount = $daylastcount = 0;
   $weekcount = $weeklastcount = $week3count = $week2count = 0;
   $monthcount = $monthlastcount = $month2count = 0;
...
/* 			} elseif($action['source'] == 'fromdate') {
				foreach($history as $operation) {
					...
					if($timestamp >= mktime(0, 0, 0, $ADate[1], $ADate[2], $ADate[0])) {
						...
						if($timestamp<=$now && $timestamp>=($now-7*86400)) {
							$weekcount++;
						}
						if($timestamp<=$now && $timestamp>=($now-30*86400)) {
							$monthcount++;
						}
					}
				} */
