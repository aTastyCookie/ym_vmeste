...
class ActionsController extends Ymoney_Controller_Action {
    ...
    protected function initFB($act) {
        ...
        $auth_url = "http://www.facebook.com/dialog/oauth?client_id=" . $config->ymoney->APP_ID . "&client_secret=" .
                ...
                urlencode($config->ymoney->APP_PAGE) . "&scope=email,publish_stream,user_photos,manage_pages";
...
//		        	echo $ymoneyNamespace->fb;  exit;
                    ...
                    //echo "<!---";
                    echo $facebook->setExtendedAccessToken();
                    //echo "---!>";
    ...
    public function saveuser() {
        ...
        if (!$userModel->find($user_id)->current()) {
			...
			echo "<script>alert(\"Application cannot get an access to user's profile.\");</script>";
			return false;
		}
        return true;
    }
...
//            }
            ...
            $toplink = ($actions[$k]['top'] == 'Y' && $admin) ? '<img src="/i/symbol-favi-24.png" alt="TOP collection"  class="mb_ico" />' : '';
            $blockedlink = ($actions[$k]['blocked'] == 'Y' && $admin) ? '<img src="/i/symbol-rejected-24.png" alt="This collection has been blocked by the administrator"  class="mb_ico" />' : '';
            ...
            $required_sumF = ($actions[$k]['required_sum'] > 0) ? 'of <span>' . $actions[$k]['required_sumF'] . '</span>' : '';
            ...
            $jsonData['li'][] = '<div class="mini_block_outer' . $last . '">
				<div class="mini_block">
					<div class="mini_block_inner">
						...
						<div class="mb_collected" id="mb_collected' . $actions[$k]['id'] . '">Collected <span>' . $actions[$k]['current_sumF'] . '</span> ' . $required_sumF . ' <span>rub.</span></div>
<a id="mb_help' .
                    $actions[$k]['id'] . '" href="https://money.yandex.ru/direct-payment.xml?receiver=' .
                    $actions[$k]['receiver'] . '&Formcomment=' .
                    urlencode('For collection in Facebook «' . $actions[$k]['name'] . '»') . '&destination=' .
                    urlencode('For collection «'.$actions[$k]['name'].'» from user facebook.com/' . $user_id . '. To send money anonymously, please delete all comments.') . '&_openstat=socapp;fben;p2p;wid&label=fben_' . $actions[$k]['id'] .'" target="_blank" class="mb_help">Donate</a>
                    <a data-id="' . $actions[$k]['id'] . '" title="Publish" style="display:none;" href="#" class="mb_shelp"></a>
					</div>
				</div>
			</div>';
            $i++;
...
<a id="mb_help' .
    ...
    public function accountAction() {
        ...
        $this->view->balance = $account['balance'] . ' rub.';
        $user_profile = $facebook->api('/' . $user_id . '/', 'GET');
        if ($user_profile) {
            $this->view->email = $user_profile['email'];
        } else {
            $this->view->error = "Access to user's information is denied";
        }
    }
...
//            die(var_dump($params));
            ...
            if (is_int($saveresult) && $saveresult > 0 && $this->share_permissions == 1) {
                ...
                $tmpObj->name = 'Link to collection';
                $tmpObj->link = $config->ymoney->APP_PAGE . '?app_data=' . $saveresult;
                $post_details = array(
                    'message' => 'Help support a good cause!',
                    'name' => $params['name'],
                    'link' => $config->ymoney->APP_PAGE . '?app_data=' . $saveresult,
                    'caption' => 'Fundraising collection',
                    'description' => $params['description'] != "" ? $this->_descriptionCut($params['description']) : "",
                    'picture' => $config->ymoney->host . "/i/app_75_new.png",
                    'actions' => $tmpObj
                );
                if($params['who']) {
                    $page_info = $facebook->api('/' . $authorArray[0]['page_id'] . '?fields=access_token');
        ...
        if ($saveFlag) {
            ...
            if (isset($params['photo']) && count($params['photo']) > 0) {
                ...
                foreach ($photos as $photo) {
                    $fql = "SELECT src, src_big FROM photo WHERE pid ='" . $photo . "'";
                    $param = array(
                        'method' => 'fql.query',
                        'query' => $fql,
                        'callback' => ''
...
//                        }
                        ...
                        if ($action['group'] == 1) {
                            ...
                            $this->view->redirect_friends_url = 'https://money.yandex.ru/direct-payment.xml?receiver=' .
                                    ...
                                    '&sum=' . ($action['required_sum_group']) . '&from=sfbp2p&_openstat=socapp;fben;p2p;list&label=fben_' . $action['id'];
                        } else {
                            $action['required_sum_group'] = 0;
                        }
