function settime() {
    ...
    if($('#signal').prop('checked')) {	
        $('.mb_timer').html('From ' + new_time1_day + ' ' + monthNamesShort[new_time1]);
    ...
    $('#cash_variants li').each(function(CVI) {
    	...
    	if($(this).hasClass('curr') && CVI==0) {
        	...
        	if($('#required_sum').val()>0) {
    			$('.right .mb_collected').html('Collected <span class="s1">'+(($('#current_sum').val()*1)+bal)+'</span> of <span class="s2">' + ($('#required_sum').val()*1) + '</span> <span>rub.</span>');
    		} else {
    			$('.right .mb_collected').html('Collected <span class="s1">'+(($('#current_sum').val()*1)+bal)+'</span> <span>rub.</span>');
    		}
    	}
