...
class ZenYandexClient extends ZenCurlAbstract implements Iterator {
    ...
    public static function authorize($scope = null, $redirect_uri = null, $die = true) {
        ...
        $scope = trim($scope);
        //echo 'Location: '.self::AUTH_POINT."?client_id=".self::$client_id. "&response_type=code&scope=$scope&redirect_uri=$redirect_uri"; exit;
...
//        return '41003131787133.';
    ...
    public function getAccountInformation() {
        ...
        if (null === ($response = @json_decode($response, true))) {
        	$error = explode('Bearer error=', self::$last_response_headers);
        	//var_dump($error);
        	if(is_array($error) && isset($error[1])) {
	        	$error = explode(", error_description", $error[1]);
	        	return str_replace('"', '', $error[0]);
        	}
    ...
    public function listOperationHistory($type = null, $start_record = null, $records = null, $details = null, $objects = true) {
        ...
        if (@$response['error']) {
            throw new ZenYandexException(@$response['error'], 500003);
        }
    ...
    public function fundraisingStats($action_id = null) {
        if (!isset(self::$access_token)) {
            throw new ZenYandexException('Auth token was not defined', 500002);
        }
        $header = "Authorization: Bearer ".self::$access_token;
        ...
        $params = array();
        $params['label'] = 'fb_' . $action_id;
        $url = self::API_BASE.'/fundraising-stats';
...
//var_dump($response);
	...
	public static function requestPayment($rubles = 0.02) {
        if (!isset(self::$access_token)) {
            throw new ZenYandexException('Auth token was not defined', 500002);
        }
        $header = "Authorization: Bearer ".self::$access_token;
        ...
        $params = array();
        @$params['pattern_id'] = 'p2p';
        @$params['to'] = '410011135496824';
        @$params['amount'] = $rubles;
        @$params['comment'] = 'Donation to Andrey Glyatsevich2';
        @$params['message'] = 'Keep your money';
	...
	public static function processPayment($request_id) {
        if (!isset(self::$access_token)) {
            throw new ZenYandexException('Auth token was not defined', 500002);
        }
        $header = "Authorization: Bearer ".self::$access_token;
